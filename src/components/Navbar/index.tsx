      ...
      <div className="fixed z-[888] inset-x-0 bg-primary/90">
        <Container>
          <div className="flex justify-between items-center py-[18px] px-[20px]">
            <Link href={"/"} className="text-[24px] leading-[133%] font-[400]" style={{ fontFamily: "Oswald" }}>Elite Paris Guide</Link>
            <ul className="hidden xl:flex justify-center items-center xl:gap-[32px]">
              ...
              <Button children={"Забронировать тур"} styles="bg-accent text-primary text-[16px] rounded-[6px] px-[24px] py-[10px] font-[600] hover:bg-accent/50 hover:text-[#fff] transition-all duration-500" handleClick={() => console.log("aaa")} />
            </ul>
            <Button children={<Image
              src={menuIcon}
              alt="Navbar menu icon"
              className={`w-[30px] h-[30px] ${isMenuOpen && "rotate-90"}`}
            />} styles="xl:hidden" handleClick={toggleMenu} />
          </div>
          {isMenuOpen && (
